...
size_data_msd=size_data_unmod %>% 
  group_by(Treatment) %>% 
  summarise("Mean Length (mm)"=round(mean(Length.mm, na.rm=T),4),
            "SD Length"=round(sd(Length.mm, na.rm=T),4), 
...
```{r}
## need to cross check data with RNASeq AND PhD THESIS****
...
top <- ggarrange(gs, ln_wt, labels=c("A", "B"))
...
## save
ggsave(plot=combo, filename="figuresTables/Figure [phenoV3].pdf",
       device="pdf", width=8.75, height=8, units=c("in"))
ggsave(plot=combo, filename="figuresTables/Figure [phenoV3].png",
       device="pdf", width=8.75, height=8, units=c("in"))
```
